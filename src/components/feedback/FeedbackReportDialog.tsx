...
export function FeedbackReportDialog({ open, onOpenChange }: Props) {
  ...
  const setupRef = useRef<RecorderSetup | null>(null);
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const showFormModal = open && !recordingSession && !enviado;
  const showSuccessModal = open && enviado;
  ...
  const syncCameraPreview = () => {
    setCameraStream(recorderRef.current?.cameraPreviewStream ?? null);
  };
  ...
  const addFiles = (files: FileList | File[]) => {
    ...
    setAttachments((prev) => {
      const room = FEEDBACK_MAX_ANEXOS - prev.length;
      if (room <= 0) {
      ...
      for (const file of list.slice(0, room)) {
        ...
        next.push(createLocalAttachment(file, isVideo ? "gravacao" : "arquivo"));
      }
      return next;
    });
  };
  ...
  const finishRecording = async () => {
    ...
    if (result && result.blob.size > 0) {
      ...
      setAttachments((prev) => {
        if (prev.length >= FEEDBACK_MAX_ANEXOS) {
          toast.error(`Máximo de ${FEEDBACK_MAX_ANEXOS} anexos por feedback.`);
          return prev;
        }
        toast.success("Gravação adicionada ao feedback.");
        return [...prev, createLocalAttachment(file, "gravacao", result.durationSeconds)];
      });
    }
  ...
  const startRecordingFlow = async (setup: RecorderSetup) => {
    ...
    setMicAvailable(setup.useMic);
    setCameraEnabled(setup.useCamera);
  ...
  return (
    <>
      <Dialog open={showFormModal} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Enviar feedback</DialogTitle>
            <DialogDescription>
              Nos ajude a melhorar o sistema — reporte um problema técnico ou envie sugestões.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col items-center text-center py-6 gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground max-w-sm">
                Recebemos sua mensagem e vamos analisar em breve. Se for um bug, nossa equipe
                trabalhará na correção o mais rápido possível.
              </p>
            </div>
      ...
      <FeedbackRecorderSetupDialog
        open={setupOpen && !recordingSession}
        onOpenChange={setSetupOpen}
        onStart={startRecordingFlow}
        loading={preparingRecording}
      />

      {recordingSession && <FeedbackCountdownOverlay value={countdown} />}
        ...
        <>
          <FeedbackRecordingToolbar
            ...
            onToggleCamera={() => {
              ...
              if (!rec) return;
              if (next) {
                void rec.enableCamera(setupRef.current?.cameraDeviceId).then(() => {
                  syncCameraPreview();
                }).catch(() => {
                  setCameraEnabled(false);
          ...
          />
          {cameraEnabled && <FeedbackCameraPreview stream={cameraStream} />}
        </>
      )}
    </>
  );
