-- Remove perfil visualizador do ERP + RLS anon para Realtime no portal do cliente

-- ── Migrar registros legados ─────────────────────────────────────────────────

update public.user_transportadoras set role = 'operador' where role = 'visualizador';
update public.transportadora_convites set role = 'operador' where role = 'visualizador';

alter table public.user_transportadoras drop constraint if exists user_transportadoras_role_check;
alter table public.user_transportadoras
  add constraint user_transportadoras_role_check check (role in ('owner', 'operador'));

alter table public.transportadora_convites drop constraint if exists transportadora_convites_role_check;
alter table public.transportadora_convites
  add constraint transportadora_convites_role_check check (role in ('owner', 'operador'));

-- ── RPCs: só owner / operador ────────────────────────────────────────────────
...
begin
  ...
  if p_role <> 'owner' and exists (
    ...
    raise exception 'Não é possível rebaixar a si mesmo de proprietário';
  end if;
...
-- ── Realtime portal cliente (anon): leitura quando há link ativo ─────────────
